...
public class Updater implements de.komoot.photon.Updater {
    ...
    public void create(PhotonDoc doc) {
        try {
            this.bulkRequest.add(this.esClient.prepareIndex("photon", "place").setSource(Utils.convert(doc, this.languages, false)).setId(String.valueOf(doc.getPlaceId())));
        } catch (IOException e) {
    ...
    public void update(PhotonDoc doc) {
        try {
            this.bulkRequest.add(this.esClient.prepareUpdate("photon", "place", String.valueOf(doc.getPlaceId())).setDoc(Utils.convert(doc, this.languages, false)));
        } catch (IOException e) {
